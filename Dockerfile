...
RUN set -eux; \
    ...
    #
    # The help output is the authoritative, side-effect-free capability check:
    # it needs no server connection. mydumper exits non-zero for --help on some
    # builds, so the exit status is ignored and only the option text is asserted.
